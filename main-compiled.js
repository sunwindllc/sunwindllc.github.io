$(document).ready(function () {
  ...
  $("#add_array").on("click", function () {
    ...
    if (array_index < max_array_index) {
      ...
      $(".array:last > div input").appendAttr("id", array_index).appendAttr("name", array_index);
      $(".array:last > div select").appendAttr("id", array_index).appendAttr("name", array_index);
      if (array_index == max_array_index) {
  ...
  $("#form").submit(function (event) {
    ...
    $.when.apply(null, promises).done(function () {
      ...
      let data = ac_monthly_array[0].map((a, b) => ac_monthly_array.map(c => c[b]).reduce((a, b) => a + b)).map((a, b) => [format_month(parse_month(b + 1)), a]);

      const ac_monthly_consumed = [$("#usage_data_01").val(), $("#usage_data_02").val(), $("#usage_data_03").val(), $("#usage_data_04").val(), $("#usage_data_05").val(), $("#usage_data_06").val(), $("#usage_data_07").val(), $("#usage_data_08").val(), $("#usage_data_09").val(), $("#usage_data_10").val(), $("#usage_data_11").val(), $("#usage_data_12").val()];

      data.map((a, b, c) => {
        c[b].push(+ac_monthly_consumed[b]);
      });
      ...
      system_summary = {
        ...
        "Payback period": (payback["Index"] + 1) % 12 == 0 ? Math.floor((payback["Index"] + 1) / 12) + " years" : (payback["Index"] + 1) % 12 == 1 ? Math.floor((payback["Index"] + 1) / 12) + " years, 1 month" : Math.floor((payback["Index"] + 1) / 12) + " years, " + (payback["Index"] + 1) % 12 + " months",
...
function render(system_summary, data_first_year, data_monthly, data_quarterly, data_annual) {
  ...
  let tbody = div.append("table").attr("class", "table").append("tbody");

  let tr = tbody.selectAll("tr").data(Object.entries(system_summary)).enter().append("tr");

  tr.append("td").attr("class", "col-xs-6").text(d => d[0] + ":");
  tr.append("td").attr("class", "col-xs-6").text(d => d[1]);
  ...
  let z = d3.scaleOrdinal().range(["#ff8c00", "#98abc5"]),
      radius = Math.min(width, height) / 2;

  var path = d3.arc().outerRadius(radius - 10).innerRadius(radius - 60);
  ...
  arc = g.selectAll(".arc").data(pie(dataPie)).enter().append("g").attr("class", "arc");

  arc.append("path").attr("d", path).data(dataPie).style("fill", function (d) {
    return z(d);
  });

  g.append("text").attr("text-anchor", "middle").attr("dy", "-1.5em").text("This system");
  g.append("text").attr("text-anchor", "middle").text("will provide for " + format_percent(d3.sum(data_first_year, d => d["Energy Generated"]) / d3.sum(data_first_year, d => d["Energy Consumed"])) + " of");
  g.append("text").attr("dy", "1.4em").attr("text-anchor", "middle").text("your energy needs.");
  ...
  margin = { top: 20, right: 20, bottom: 30, left: 40 }, width = +svg.attr("width") - margin.left - margin.right, height = +svg.attr("height") - margin.top - margin.bottom, g = svg.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
  ...
  z = d3.scaleOrdinal().range(["#ff8c00", "#98abc5"]);

  var keys = ["Energy Generated", "Energy Consumed"];

  x0.domain(data_first_year.map(function (d) {
    return d.Month;
  }));
  x1.domain(keys).rangeRound([0, x0.bandwidth()]);
  y.domain([0, d3.max(data_first_year, function (d) {
    return d3.max(keys, function (key) {
      return d[key];
    });
  })]).nice();

  g.append("g").selectAll("g").data(data_first_year).enter().append("g").attr("transform", function (d) {
  ...
  g.append("g").attr("class", "axis").call(d3.axisLeft(y).ticks(null)).append("text").attr("x", 2).attr("y", y(y.ticks().pop()) + 0.5).attr("dy", "0.32em").attr("fill", "#000").attr("font-weight", "bold").attr("text-anchor", "start").text("kWh");
  ...
  z = d3.scaleOrdinal().range(["#ff8c00", "#98abc5"].reverse());
  ...
  g.append("g").attr("class", "axis").call(d3.axisLeft(y).ticks(null).tickFormat(function (d) {
    return this.parentNode.nextSibling ? "\xa0" + d : "$" + d;
  }));

  legend = g.append("g").attr("font-family", "sans-serif").attr("font-size", 10).attr("text-anchor", "end").selectAll("g").data(keys.slice()).enter().append("g").attr("transform", function (d, i) {
    return "translate(0," + i * 20 + ")";
  });

  legend.append("rect").attr("x", width - 19).attr("width", 19).attr("height", 19).attr("fill", z);

  legend.append("text").attr("x", width - 24).attr("y", 9.5).attr("dy", "0.32em").text(d => d);
  ...
  margin = { top: 10, right: 10, bottom: 20, left: 50 }, width = +svg.attr("width") - margin.left - margin.right, height = +svg.attr("height") - margin.top - margin.bottom, g = svg.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

  y = d3.scaleLinear().domain(d3.extent(data_monthly, d => d["System value"])).nice().rangeRound([height, 0]);

  x = d3.scaleTime().domain(d3.extent(data_monthly, d => parse(d["Month"]))).rangeRound([0, width]);

  let area = d3.area().x(d => x(parse(d["Month"]))).y1(d => y(d["System value"])).y0(y(0)),
      line = d3.line().x(d => x(parse(d["Month"]))).y(d => y(d["System value"]));

  svg.append("svg:clipPath").attr("id", "clipTop").append("svg:rect").attr("height", y(0)).attr("width", width);

  svg.append("svg:clipPath").attr("id", "clipBottom").append("svg:rect").attr("height", height - y(0)).attr("width", width).attr("y", y(0));

  g.append("path").datum(data_monthly).attr("d", area).attr("class", "positive").attr("clip-path", "url(#clipTop)");

  g.append("path").datum(data_monthly).attr("d", area).attr("class", "negative").attr("clip-path", "url(#clipBottom)");

  g.append("path").datum(data_monthly).attr("d", line).attr("class", "line").attr("stroke-dasharray", "4,2").attr("stroke-width", "0.5");

  g.append("g").attr("class", "axis").attr("transform", "translate(0," + y(0) + ")").call(d3.axisBottom(x));

  g.append("g").attr("class", "axis").call(d3.axisLeft(y).ticks(null).tickFormat(function (d) {
    let formatNumber = d3.format(",");
    return this.parentNode.nextSibling ? "\xa0" + formatNumber(d) : "$" + formatNumber(d);
  })).append("text").attr("x", 2).attr("y", y(y.ticks().pop()) + 0.5).attr("dy", "0.32em").attr("fill", "#000").attr("font-weight", "bold").attr("text-anchor", "start").text("Revenue");
}
